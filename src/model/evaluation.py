...
def inference_and_calculate_accuracy(
    model,
    tokenizer,
    dataset,
    max_length=512,    
    ...
    scores = []
    records = []
    for sample in tqdm(dataset, desc=f"Evaluating model..."):

        input_text = prompt.generate_llm_input(
            sample["vignette"], sample["drug"])
...
def get_score(reference, prediction):
    ...
    if "dosage" in prediction.keys():
        # normalise "Low" vs "Low (10mg)" vs "Low (1week)"
        pred_dosage = prediction["dosage"].split(" ")[0].lower()
...
def evaluate(models:dict, 
    ...
    for model_type, model_name_path in models.items():
        ...
        for dataset_name, dataset in test_datasets.items():
            ...
            average_score, details = inference_and_calculate_accuracy(
            ...
            details.to_csv(
                f"eval_{dataset_name}_by_{formatted_model_name}_score_{average_score:.2f}_ts_{ts}.csv")
